// Copyright (c) 2021, Prime Holding JSC
// https://www.primeholding.com
//
// Use of this source code is governed by an MIT-style
// license that can be found in the LICENSE file or at
// https://opensource.org/licenses/MIT.

{{#push_notifications}}
import 'package:firebase_messaging/firebase_messaging.dart'; {{/push_notifications}}
import 'package:rx_bloc/rx_bloc.dart';
import 'package:rxdart/rxdart.dart';

import '../../base/repositories/push_notification_subscription_repository.dart';

part 'send_notifications_bloc.rxb.g.dart';
part 'send_notifications_bloc_extensions.dart';

/// A contract class containing all events of the SendNotificationsBloC.
abstract class SendNotificationsBlocEvents {
  /// Requests permissions for displaying push notifications
  void requestNotificationPermissions();

  /// Issues a new push message
  void sendMessage(String message, {String? title, int? delay});
}

/// A contract class containing all states of the SendNotificationsBloC.
abstract class SendNotificationsBlocStates {
  /// Are the permissions for displaying push notifications granted
  Stream<bool> get permissionsAuthorized;
}

@RxBloc()
class SendNotificationsBloc extends $SendNotificationsBloc {
  SendNotificationsBloc(this._notificationsRepo);

  final PushNotificationSubscriptionRepository _notificationsRepo;

  @override
  Stream<bool> _mapToPermissionsAuthorizedState() => Rx.merge([
        _$sendMessageEvent
            .switchMap((args) => _sendMessage(args).asResultStream()),
        _$requestNotificationPermissionsEvent
            .switchMap((_) => _getAuthStatus().asResultStream()),
      ]).setResultStateHandler(this).whereSuccess();
}
